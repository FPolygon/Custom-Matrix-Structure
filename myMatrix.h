...
public:
  ...
  mymatrix(int R, int C)
  {
    ...
    //
      ...
      Rows = new ROW[R];  // allocate array of ROWs
          NumRows = R;

          // Initialize each row with numCols columns:
          for (int i = 0; i < NumRows; ++i)
          {
            Rows[i].Cols = new T[C];  // allocate array of columns
            Rows[i].NumCols = C;

            // Initialize the elements to their default value:
            for (int j = 0; j < Rows[i].NumCols; ++j)
            {
              Rows[i].Cols[j] = T{};  // default value for type T:
            }
          }
  }
  ...
  //
    mymatrix(const mymatrix<T>& other) {
        ...
        // Initialize each row with the same number of columns as in the other matrix:
        for (int r = 0; r < NumRows; ++r) {
            Rows[r].Cols = new T[other.Rows[r].NumCols];
            Rows[r].NumCols = other.Rows[r].NumCols;

            // Copy the elements of the other matrix into the new matrix:
            for (int c = 0; c < Rows[r].NumCols; ++c) {
